...
// --------  Another Solution for getEscapedChars() method ------------

        /**private String getEscapedChars() {
        String bracketsRemoved = removeBrackets(getCustomDelimiters());
        String[] delimitersToEscape = bracketsRemoved.split(PIPE_DELIMITER_ESCAPED);
        return Arrays.stream(delimitersToEscape)
                .map(Pattern::quote)
